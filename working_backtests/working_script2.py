...
bt = Backtest(strategy=WeightedIndicatorStrategy, data=GOOG, cash=10000, commission=0.002, margin=1.0, exclusive_orders=True)
result = bt.optimize(
    buy_threshold=range(30, 60, 5),  
    sell_threshold=range(20, 50, 5),  
    stop_loss_threshold=range(5, 15, 2),
    maximize='# Trades',
    constraint=lambda p: p.buy_threshold > p.sell_threshold  # Adding constraints
)
